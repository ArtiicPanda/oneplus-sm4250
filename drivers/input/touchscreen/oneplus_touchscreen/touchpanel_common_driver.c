...
uint8_t DouTap_enable;				 // double tap
uint8_t UpVee_enable;				 // V
uint8_t LeftVee_enable;				// >
uint8_t RightVee_enable;			 // <
uint8_t Circle_enable;				 // O
uint8_t DouSwip_enable;				// ||
uint8_t Mgestrue_enable;			 // M
uint8_t Wgestrue_enable;			 // W
uint8_t Sgestrue_enable;			 // S
uint8_t SingleTap_enable;			 // single tap
uint8_t Enable_gesture;
int sigle_num;
...
static void tp_gesture_handle(struct touchpanel_data *ts)
{
...
#endif //end of CONFIG_ONEPLUS_TP_APK
	...
	if ((gesture_info_temp.gesture_type == DouTap && DouTap_enable) ||
		...
		if (ts->geature_ignore)
			return;
#endif
	TPD_INFO("gesture_switch_value = %d\n", gesture_switch_value);
	if (gesture_switch_value != 2) {
		input_report_key(ts->input_dev, KEY_F4, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_F4, 0);
		input_sync(ts->input_dev);
	} else {
		TPD_INFO("psensor is cover\n");
	}
...
static void tp_btnkey_release(struct touchpanel_data *ts)
{
    if (CHK_BIT(ts->vk_bitmap, BIT_MENU))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_MENU, 0);
    if (CHK_BIT(ts->vk_bitmap, BIT_HOME))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_HOMEPAGE, 0);
    if (CHK_BIT(ts->vk_bitmap, BIT_BACK))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_BACK, 0);
...
static void tp_btnkey_handle(struct touchpanel_data *ts)
{
    ...
    if (CHK_BIT(ts->vk_bitmap, BIT_MENU))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_MENU, CHK_BIT(touch_state, BIT_MENU));
    if (CHK_BIT(ts->vk_bitmap, BIT_HOME))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_HOMEPAGE, CHK_BIT(touch_state, BIT_HOME));
    if (CHK_BIT(ts->vk_bitmap, BIT_BACK))
        input_report_key_oneplus(ts->kpd_input_dev, KEY_BACK, CHK_BIT(touch_state, BIT_BACK));
    input_sync(ts->kpd_input_dev);
}
...
static ssize_t proc_gesture_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ts->is_incell_panel && (ts->suspend_state == TP_RESUME_EARLY_EVENT || ts->disable_gesture_ctrl ||
		get_lcd_status() > 0) && (ts->tp_resume_order == LCD_TP_RESUME)) {
...
static ssize_t cap_vk_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if (!g_tp)
	return snprintf(buf, 13, "not support");
...
//proc/touchpanel/oneplus_register_info node use info:
//first choose register_add and lenght, example: echo 000e,1 > oneplus_register_info
//second read: cat oneplus_register_info
...
//write info: echo 000e,1 > oneplus_register_info
...
static ssize_t oneplus_apk_read(struct file *file,
...
static ssize_t oneplus_apk_write(struct file *file,
...
static const struct file_operations proc_oneplus_apk_fops = {
    .owner = THIS_MODULE,
    .open = simple_open,
    .read = oneplus_apk_read,
    .write = oneplus_apk_write,
};
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->fw_update_app_support)
		tp_manufac_devinfo = ts->panel_data.manufacture_info;
	...
	//proc files-step2-2:/proc/touchpanel/oneplus_tp_fw_update (FW update interface)
	...
	//proc files-step2-3:/proc/touchpanel/oneplus_tp_fw_update (edge limit control interface)
	if (ts->edge_limit_support || ts->fw_edge_limit_support) {
	...
	prEntry_tmp = proc_create_data("test_limit_enable", 0664, prEntry_tp, &touch_dir_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}

	prEntry_tmp = proc_create_data("oneplus_tp_limit_whitelist", 0666, prEntry_tp, &limit_valid_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
	...
	if (ts->edge_limit_support) {
		prEntry_tmp = proc_create_data("oneplus_tp_limit_area", 0664, prEntry_tp, &proc_limit_area_ops, ts);
		if (prEntry_tmp == NULL) {
			ret = -ENOMEM;
			TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
		}
	...
	if (ts->black_gesture_support) {
		prEntry_tmp = proc_create_data("gesture_enable", 0664, prEntry_tp, &proc_gesture_control_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
    ...
    //proc files-step2-11:/proc/touchpanel/oneplus_tp_noise_modetest
    if (ts->noise_modetest_support) {
        prEntry_tmp = proc_create_data("oneplus_tp_noise_modetest", 0664, prEntry_tp, &proc_noise_modetest_fops, ts);
        if (prEntry_tmp == NULL) {
            ret = -ENOMEM;
            TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
        }
...
#ifdef CONFIG_ONEPLUS_TP_APK
    // proc/touchpanel/oneplus_apk. Add the new test node for debug and apk. By zhangping 20190402 start
    prEntry_tmp = proc_create_data("oneplus_apk", 0666,
                                   prEntry_tp, &proc_oneplus_apk_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n",
                 __func__, __LINE__);
...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
    ...
    if (IS_ERR_OR_NULL(ts->hw_res.pinctrl)) {
        ...
        ts->hw_res.pin_set_low = pinctrl_lookup_state(ts->hw_res.pinctrl, "state_rst_output0");
		if (IS_ERR(ts->hw_res.pin_set_low)) {
			rc  = PTR_ERR(ts->hw_res.pin_set_low);
			printk("Cannot find pinctrl rst output0 %d!\n", rc );
		}

        ts->hw_res.pin_set_high= pinctrl_lookup_state(ts->hw_res.pinctrl, "state_rst_output1");
		if (IS_ERR(ts->hw_res.pin_set_high)) {
			rc  = PTR_ERR(ts->hw_res.pin_set_high);
			printk("Cannot find pinctrl rst output1 %d!\n", rc );
		}
...
static void speedup_resume(struct work_struct *work)
{
    ...
    if (!ts->skip_reset_in_resume && !ts->fp_info.touch_state) {
		TPD_INFO("ts->skip_reset_in_resume = %d, ts->fp_info.touch_state =%d",ts->skip_reset_in_resume, ts->fp_info.touch_state);
        if (!ts->lcd_trigger_load_tp_fw_support) {
            ts->ts_ops->reset(ts->chip_data);
        }
    }
        ...
        if (*blank == FB_BLANK_POWERDOWN) { //suspend
            ...
            if (event == FB_EARLY_EVENT_BLANK) {    //early event
                ...
                if (ts->tp_resume_order == TP_LCD_RESUME) {
                    ...
                    tp_resume(ts->dev);
                    if (!ts->irq_trigger_hdl_support) {
                        d = irq_desc_get_irq_data(irq_to_desc(ts->irq));
...
/**
 * input_report_key_oneplus - Using for report virtual key
 * @work: work struct using for this thread
 *
 * before report virtual key, detect whether touch_area has been touched
 * Do not care the result: Return void type
 */
void input_report_key_oneplus(struct input_dev *dev, unsigned int code, int value)
{
...
static int oneplus_tp_spi_suspend(struct device *dev)
{
    struct touchpanel_data *ts = dev_get_drvdata(dev);
	...
	if ((ts->boot_mode == META_BOOT || ts->boot_mode == FACTORY_BOOT))
#else
	if ((ts->boot_mode == MSM_BOOT_MODE_FACTORY ||
		ts->boot_mode == MSM_BOOT_MODE_RF))
#endif
    {
...
static int oneplus_tp_spi_resume(struct device *dev)
{
    struct touchpanel_data *ts = dev_get_drvdata(dev);
...
static struct of_device_id tp_match_table_hx83102d[] = {
    { .compatible = MATCH_HIMAX_83102d,},
    { },
};
#endif
...
extern int __maybe_unused ilitek9882n_spi_probe(struct spi_device *spi);
extern int __maybe_unused ilitek9882n_spi_remove(struct spi_device *spi);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET
extern int __maybe_unused hx83102d_tp_probe(struct spi_device *spi);
extern int __maybe_unused hx83102d_tp_remove(struct spi_device *spi);
#endif


static int32_t __init oneplus_tp_driver_init(void)
{
    TPD_INFO("%s is called\n", __func__);
    ...
    case ILI9881H:
        ...
        break;
    case ILI9882N:
        TPD_INFO("%s case ILI9882N \n", __func__);
        tp_spi_driver.driver.of_match_table = tp_match_table_ili;
        tp_spi_driver.probe = ilitek9882n_spi_probe;
        tp_spi_driver.remove = ilitek9882n_spi_remove;
        break;
#endif
    ...
    case NT36525B:
		...
		printk("touchpanel: add by wanglongfei-0210 himax\n");

#ifdef CONFIG_TOUCHSCREEN_HIMAX_CHIPSET
		case HX83102D:
			printk("touchpanel: add by wanglongfei-0210: Enter	%d\n", HX83102D);
			tp_spi_driver.driver.of_match_table = tp_match_table_hx83102d;
			tp_spi_driver.probe = hx83102d_tp_probe;
			tp_spi_driver.remove = hx83102d_tp_remove;
			break;
#endif


    default:
	TPD_INFO("%s default return -1 \n", __func__);
        return -1;
        break;
    }
...
static void __exit oneplus_tp_driver_exit(void)
{
    spi_unregister_driver(&tp_spi_driver);
}

module_init(oneplus_tp_driver_init);
module_exit(oneplus_tp_driver_exit);

MODULE_DESCRIPTION("ONEPLUS Touchscreen Driver");
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Ping.Zhang<zhangping.a@oneplus.com>");
