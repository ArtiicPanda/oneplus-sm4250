...
#include "oneplus_short.h"
#include "oneplus_adapter.h"
#include "charger_ic/oneplus_short_ic.h"
#include "../../thermal/thermal_core.h"

#ifdef CONFIG_ONEPLUS_EMMC_LOG
/*Jingchun.Wang@BSP.Kernel.Debug, 2016/12/21,*/
/*add for emmc log*/
//#include <soc/oneplus/oneplus_emmclog.h>//debug for bring up 
#endif /*CONFIG_ONEPLUS_EMMC_LOG*/

static struct oneplus_chg_chip *g_charger_chip = NULL;

#define ONEPLUS_CHG_UPDATE_INTERVAL_SEC 		5
/* first run after init 10s */
#define ONEPLUS_CHG_UPDATE_INIT_DELAY	round_jiffies_relative(msecs_to_jiffies(500))
/* update cycle 5s while charger on*/
#define ONEPLUS_CHG_UPDATE_INTERVAL	round_jiffies_relative(msecs_to_jiffies(ONEPLUS_CHG_UPDATE_INTERVAL_SEC*1000))
#define ONEPLUS_CHGON_WAKEUP_DELAY_COUNT 4  //20S
#define ONEPLUS_CHG_DEFAULT_CHARGING_CURRENT	512
#define ONEPLUS_BAT_TEMP	440
...
void oneplus_chg_turn_off_charging(struct oneplus_chg_chip *chip);
void oneplus_chg_turn_on_charging(struct oneplus_chg_chip *chip);

static void oneplus_chg_variables_init(struct oneplus_chg_chip *chip);
static void oneplus_chg_update_work(struct work_struct *work);
static void oneplus_chg_protection_check(struct oneplus_chg_chip *chip);
static void oneplus_chg_get_battery_data(struct oneplus_chg_chip *chip);
static void oneplus_chg_check_tbatt_status(struct oneplus_chg_chip *chip);
static void oneplus_chg_get_chargerid_voltage(struct oneplus_chg_chip *chip);
static void oneplus_chg_set_input_current_limit(struct oneplus_chg_chip *chip);
static void oneplus_chg_battery_update_status(struct oneplus_chg_chip *chip);
...
enum power_supply_property oneplus_usb_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
	POWER_SUPPLY_PROP_OTG_SWITCH,
	POWER_SUPPLY_PROP_OTG_ONLINE,
};

enum power_supply_property oneplus_ac_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
#ifdef CONFIG_ONEPLUS_FAST2NORMAL_CHG
	POWER_SUPPLY_PROP_FAST2NORMAL_CHG,
#endif
};

enum power_supply_property oneplus_batt_props[] = {
...
#endif
#ifndef CONFIG_ONEPLUS_SDM670_CHARGER
...
#ifdef CONFIG_ONEPLUS_CHECK_CHARGERID_VOLT
	POWER_SUPPLY_PROP_CHARGERID_VOLT,
#endif
#ifdef CONFIG_ONEPLUS_SHIP_MODE_SUPPORT
	POWER_SUPPLY_PROP_SHIP_MODE,
#endif
#ifdef CONFIG_ONEPLUS_CALL_MODE_SUPPORT
	POWER_SUPPLY_PROP_CALL_MODE,
#endif
#ifdef CONFIG_ONEPLUS_SHORT_C_BATT_CHECK
...
#endif /*CONFIG_ONEPLUS_SHORT_USERSPACE*/
#endif
...
struct board_temp_input_config g_btemp_config[] = {
	[BOARD_TEMP_STATUS_NORMAL] = {-1000, 35, VBUS_LIMIT_9V, ONEPLUS_CHG_2000_CHARGING_CURRENT},
	[BOARD_TEMP_STATUS_LEVEL1] = {35, 37, VBUS_LIMIT_9V, ONEPLUS_CHG_1500_CHARGING_CURRENT},
	[BOARD_TEMP_STATUS_LEVEL2] = {37, 39, VBUS_LIMIT_5V, ONEPLUS_CHG_1500_CHARGING_CURRENT},
	[BOARD_TEMP_STATUS_LEVEL3] = {39, 43, VBUS_LIMIT_5V, ONEPLUS_CHG_1200_CHARGING_CURRENT},
	[BOARD_TEMP_STATUS_LEVEL4] = {43, 1000, VBUS_LIMIT_5V, ONEPLUS_CHG_1000_CHARGING_CURRENT},
};

struct battery_temp_input_config g_battery_temp_config[] = {
	[BATTERY_TEMP_STATUS_NORMAL] = {-1000, 420, VBUS_LIMIT_9V},
	[BATTERY_TEMP_STATUS_LIMIT] = {420, 1000, VBUS_LIMIT_5V},
};

struct batt_temp_zone_info batt_temp_zone_arr[BATT_STAT_INVALID];

void batt_temp_zone_arr_init(struct oneplus_chg_chip *chip)
{
	batt_temp_zone_arr[BATT_STAT_COLD].batt_temp_thres_addr = &(chip->limits.cold_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_COLD].default_value = chip->limits.cold_bat_decidegc;  //-2C
	batt_temp_zone_arr[BATT_STAT_LITTLE_COLD].batt_temp_thres_addr = &(chip->limits.little_cold_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_LITTLE_COLD].default_value = chip->limits.little_cold_bat_decidegc;  //0C
	batt_temp_zone_arr[BATT_STAT_COOL].batt_temp_thres_addr = &(chip->limits.cool_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_COOL].default_value = chip->limits.cool_bat_decidegc;  //5C
	batt_temp_zone_arr[BATT_STAT_LITTLE_COOL].batt_temp_thres_addr = &(chip->limits.little_cool_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_LITTLE_COOL].default_value = chip->limits.little_cool_bat_decidegc;  //12C
	batt_temp_zone_arr[BATT_STAT_NORMAL_0].batt_temp_thres_addr = &(chip->limits.normal_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_0].default_value = chip->limits.normal_bat_decidegc;  //16C
	batt_temp_zone_arr[BATT_STAT_NORMAL].batt_temp_thres_addr = &(chip->limits.qc_temp_cool_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL].default_value = chip->limits.qc_temp_cool_decidegc;  //22C
	batt_temp_zone_arr[BATT_STAT_NORMAL_1].batt_temp_thres_addr = &(chip->limits.qc_temp_normal_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_1].default_value = chip->limits.qc_temp_normal_decidegc;  //34C
	batt_temp_zone_arr[BATT_STAT_NORMAL_2].batt_temp_thres_addr = &(chip->limits.qc_temp_warm_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_2].default_value = chip->limits.qc_temp_warm_decidegc;  //37C
	batt_temp_zone_arr[BATT_STAT_NORMAL_3].batt_temp_thres_addr = &(chip->limits.qc_temp_little_hot_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_3].default_value = chip->limits.qc_temp_little_hot_decidegc;  //40C
	batt_temp_zone_arr[BATT_STAT_NORMAL_4].batt_temp_thres_addr = &(chip->limits.qc_temp_hot_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_4].default_value = chip->limits.qc_temp_hot_decidegc;  //42C
	batt_temp_zone_arr[BATT_STAT_WARM].batt_temp_thres_addr = &(chip->limits.warm_bat_decidegc);
	batt_temp_zone_arr[BATT_STAT_WARM].default_value = chip->limits.warm_bat_decidegc;  //45C
}

#ifdef CONFIG_ONEPLUS_CHARGER_MTK
int oneplus_usb_get_property(struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
		{
	int ret = 0;

	//struct oneplus_chg_chip *chip = container_of(psy->desc, struct oneplus_chg_chip, usb_psd);
	struct oneplus_chg_chip *chip = g_charger_chip;
...
int oneplus_usb_property_is_writeable(struct power_supply *psy,
		enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
...
void __attribute__((weak)) oneplus_set_otg_switch_status(bool value)
{
	return;
}

int oneplus_usb_set_property(struct power_supply *psy,
		enum power_supply_property psp,
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_OTG_SWITCH:
			if (val->intval == 1) {
				...
				oneplus_set_otg_switch_status(true);
			} else {
				chip->otg_switch = false;
				chip->otg_online = false;
				oneplus_set_otg_switch_status(false);
			}
...
static void usb_update(struct oneplus_chg_chip *chip)
{
	if (chip->charger_exist) {
...
int oneplus_ac_get_property(struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
	int ret = 0;
	//struct oneplus_chg_chip *chip = container_of(psy->desc, struct oneplus_chg_chip, ac_psd);
	struct oneplus_chg_chip *chip = g_charger_chip;
	...
	if (chip->charger_exist) {
		if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP)
				...
				|| (oneplus_warp_get_fastchg_started() == true)
				|| (oneplus_warp_get_fastchg_to_normal() == true)
				|| (oneplus_warp_get_fastchg_to_warm() == true)
				|| (oneplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
				|| (oneplus_warp_get_btb_temp_over() == true)) {
			chip->ac_online = true;
		} else {
			chip->ac_online = false;
		}
	} else {
		if ((oneplus_warp_get_fastchg_started() == true)
				|| (oneplus_warp_get_fastchg_to_normal() == true)
				|| (oneplus_warp_get_fastchg_to_warm() == true)
				|| (oneplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
				|| (oneplus_warp_get_btb_temp_over() == true)
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_FAST2NORMAL_CHG:
			if (oneplus_warp_get_fastchg_to_normal() == true
					|| oneplus_warp_get_fastchg_to_warm() == true
					|| oneplus_warp_get_btb_temp_over() == true
					|| oneplus_warp_get_fastchg_low_temp_full() == true) {
				val->intval = 1;
			} else {
				val->intval = 0;
			}
...
static int get_prop_chg_protect_status(struct oneplus_chg_chip *chg)
{
	if (chg->use_fake_protect_sts)
		return chg->fake_protect_sts;

	if (!chg->charger_exist)
		return 0;

	if (chg->notify_flag == NOTIFY_CHARGER_OVER_VOL)
		return PROTECT_CHG_OVP;
	else if (chg->notify_flag == NOTIFY_BAT_NOT_CONNECT)
		return  PROTECT_BATT_MISSING;
	else if (chg->notify_flag == NOTIFY_BAT_OVER_VOL)
		return PROTECT_BATT_OVP;
	else if (chg->notify_flag == NOTIFY_CHGING_OVERTIME)
		return PROTECT_CHG_OVERTIME;
	else if (chg->notify_flag == NOTIFY_BAT_OVER_TEMP)
		return PROTECT_BATT_TEMP_REGION__HOT;
	else if (chg->notify_flag == NOTIFY_BAT_LOW_TEMP)
		return PROTECT_BATT_TEMP_REGION_COLD;
	else if (chg->notify_flag == NOTIFY_BAT_FULL_PRE_HIGH_TEMP)
		return PROTECT_BATT_TEMP_REGION_LITTLE_COLD;
	else if (chg->notify_flag == NOTIFY_BAT_FULL_PRE_LOW_TEMP)
		return PROTECT_BATT_TEMP_REGION_WARM;
	else
		return 0;
}

int op_set_prop_chg_protect_status(struct oneplus_chg_chip *chg,
				  const union power_supply_propval *val)
{
...
int oneplus_battery_property_is_writeable(struct power_supply *psy,
		enum power_supply_property psp)
{
	int rc = 0;
...
int oneplus_battery_set_property(struct power_supply *psy,
		enum power_supply_property psp,
		const union power_supply_propval *val)
{
	int ret = 0;
	//struct oneplus_chg_chip *chip = container_of(psy->desc, struct oneplus_chg_chip, battery_psd);
	struct oneplus_chg_chip *chip = g_charger_chip;
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE:
			...
			if (val->intval == 0) {
				if(chip->unwakelock_chg == 1) {
					...
					chip->mmi_chg = 0;
					oneplus_chg_turn_off_charging(chip);
					if (oneplus_warp_get_fastchg_started() == true) {
						oneplus_chg_set_chargerid_switch_val(0);
						oneplus_warp_switch_mode(NORMAL_CHARGER_MODE);
						chip->mmi_fastchg = 0;
					}
				}
			} else {
				if(chip->unwakelock_chg == 1) {
					...
					chip->mmi_fastchg = 1;
					oneplus_chg_turn_on_charging(chip);
				}
			}
			break;
#ifdef CONFIG_ONEPLUS_SMART_CHARGER_SUPPORT
		case POWER_SUPPLY_PROP_COOL_DOWN:
			charger_xlog_printk(CHG_LOG_CRTI, "****************************************cool_down********************************* [0x%x].\n", val->intval);
			oneplus_smart_charge_by_cool_down(chip, val->intval);
			break;
#endif
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
		case POWER_SUPPLY_PROP_STOP_CHARGING_ENABLE:
		...
		case POWER_SUPPLY_PROP_SHIP_MODE:
			...
			oneplus_gauge_update_soc_smooth_parameter();
			break;
#endif
#ifdef CONFIG_ONEPLUS_CALL_MODE_SUPPORT
		case POWER_SUPPLY_PROP_CALL_MODE:
...
#ifdef CONFIG_ONEPLUS_SHORT_USERSPACE
		case POWER_SUPPLY_PROP_SHORT_C_LIMIT_CHG:
			printk(KERN_ERR "[ONEPLUS_CHG] [short_c_bat] set limit chg[%d]\n", !!val->intval);
			...
			break;
		case POWER_SUPPLY_PROP_SHORT_C_LIMIT_RECHG:
			printk(KERN_ERR "[ONEPLUS_CHG] [short_c_bat] set limit rechg[%d]\n", !!val->intval);
			chip->short_c_batt.limit_rechg = !!val->intval;
			break;
#else
		case POWER_SUPPLY_PROP_SHORT_C_BATT_UPDATE_CHANGE:
			printk(KERN_ERR "[ONEPLUS_CHG] [short_c_batt]: set update change[%d]\n", val->intval);
			oneplus_short_c_batt_update_change(chip, val->intval);
			chip->short_c_batt.update_change = val->intval;
			break;

		case POWER_SUPPLY_PROP_SHORT_C_BATT_IN_IDLE:
			printk(KERN_ERR "[ONEPLUS_CHG] [short_c_batt]: set in idle[%d]\n", !!val->intval);
			chip->short_c_batt.in_idle = !!val->intval;
			break;
#endif /*CONFIG_ONEPLUS_SHORT_USERSPACE*/
#endif /* CONFIG_ONEPLUS_SHORT_C_BATT_CHECK */
#ifdef CONFIG_ONEPLUS_SHORT_HW_CHECK
		case POWER_SUPPLY_PROP_SHORT_C_HW_FEATURE:
			printk(KERN_ERR "[ONEPLUS_CHG] [short_c_hw_check]: set is_feature_hw_on [%d]\n", val->intval);
		...
		case POWER_SUPPLY_PROP_SHORT_C_IC_VOLT_THRESH:
			if (chip) {
				...
				oneplus_short_ic_set_volt_threshold(chip);
				//pr_err("%s:[ONEPLUS_CHG][oneplus_short_ic],ic_volt_threshold val->intval[%d]\n", __FUNCTION__, val->intval);
			}
			break;
#endif
		case POWER_SUPPLY_PROP_TEMP:
			...
			break;
		case POWER_SUPPLY_PROP_CHG_PROTECT_STATUS:
			ret = op_set_prop_chg_protect_status(chip, val);
			break;
		default:
...
int oneplus_battery_get_property(struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
	int ret = 0;
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			if (oneplus_chg_show_warp_logo_ornot() == 1) {
		...
		case POWER_SUPPLY_PROP_HEALTH:
			val->intval = oneplus_chg_get_prop_batt_health(chip);
			break;
		...
		case POWER_SUPPLY_PROP_CURRENT_NOW:
			if (oneplus_warp_get_fastchg_started() == true) {
				chip->icharging = oneplus_gauge_get_prev_batt_current();
			} else {
				chip->icharging = oneplus_gauge_get_batt_current();
			}
			val->intval = chip->icharging;
			break;
		case POWER_SUPPLY_PROP_TEMP:
			if (chip->fake_temp != 999)
				val->intval = chip->fake_temp;
			else
				val->intval = chip->temperature - chip->offset_temp;
			break;
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			if (oneplus_warp_get_fastchg_started() == true && (chip->vbatt_num == 2)
			&& oneplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP) {
		...
		case POWER_SUPPLY_PROP_FAST_CHARGE:
			val->intval = oneplus_chg_show_warp_logo_ornot();
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
			if (val->intval) {
		...
		case POWER_SUPPLY_PROP_ADAPTER_FW_UPDATE:
			val->intval = oneplus_warp_get_adapter_update_status();
			break;
		case POWER_SUPPLY_PROP_WARPCHG_ING:
			val->intval = oneplus_warp_get_fastchg_ing();
			break;
#ifdef CONFIG_ONEPLUS_CHECK_CHARGERID_VOLT
		...
		case POWER_SUPPLY_PROP_SHORT_C_BATT_CV_STATUS:
			val->intval = (int)oneplus_short_c_batt_get_cv_status(chip);
			break;
#endif /*CONFIG_ONEPLUS_SHORT_USERSPACE*/
#endif /* CONFIG_ONEPLUS_SHORT_C_BATT_CHECK */
#ifdef CONFIG_ONEPLUS_SHORT_HW_CHECK
		...
		case POWER_SUPPLY_PROP_SHORT_C_IC_OTP_VALUE:
			if (chip) {
				val->intval = oneplus_short_ic_get_otp_error_value(chip);
			}
			if (chip->unsupport_short_c) {
				val->intval = 0;
			}
		...
		const char __user *buf, size_t len, loff_t *data)
{
	...
	if (buffer[0] == '0') {
		...
		oneplus_tbatt_power_off_task_wakeup();
	}
...
static void oneplus_chg_set_awake(struct oneplus_chg_chip *chip, bool awake);
		...
		const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		oneplus_chg_set_input_current_limit(g_charger_chip);
		...
		oneplus_chg_set_awake(g_charger_chip, true);
		g_charger_chip->chg_ops->charger_unsuspend();
		g_charger_chip->chg_ops->charging_enable();
		g_charger_chip->mmi_chg = 1;
		g_charger_chip->stop_chg = 1;
		...
		oneplus_chg_set_awake(g_charger_chip, false);
...
static void mmi_adapter_in_work_func(struct work_struct *work)
{
	...
	struct oneplus_chg_chip *chip
		= container_of(dwork, struct oneplus_chg_chip, mmi_adapter_in_work);
	chip->mmi_fastchg = 1;
	charger_xlog_printk(CHG_LOG_CRTI, "  mmi_fastchg\n");
}

static void oneplus_mmi_fastchg_in(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_awake_init(struct oneplus_chg_chip *chip)
{
	if (!chip) {
		return;
	}
...
{
	struct oneplus_chg_chip *chip = data;
	struct cred *new;
	int rc = 0;
	...
	while (!kthread_should_stop()) {
		...
		oneplus_chg_short_c_battery_check(chip);
...
int oneplus_chg_init(struct oneplus_chg_chip *chip)
{
	...
	oneplus_chg_variables_init(chip);
	oneplus_chg_get_battery_data(chip);
	...
	g_charger_chip = chip;
	oneplus_chg_awake_init(chip);
	INIT_DELAYED_WORK(&chip->update_work, oneplus_chg_update_work);
	if(!chip->unsupport_short_c){
	...
	/*ye.zhang add end*/
	schedule_delayed_work(&chip->update_work, ONEPLUS_CHG_UPDATE_INIT_DELAY);
...
int oneplus_chg_parse_swarp_dt(struct oneplus_chg_chip *chip)
{
	...
	chg_err("oneplus_parse_swarp_dt, chip->vbatt_num = %d,chip->warp_project = %d.\n",
...
int oneplus_chg_parse_charger_dt(struct oneplus_chg_chip *chip)
{
...
#ifdef CONFIG_ONEPLUS_SPECIAL__DEL
	...
	rc = of_property_read_u32(node, "qcom,qc_temp_cool_decidegc",
			&chip->limits.qc_temp_cool_decidegc);
	if (rc < 0) {
		chip->limits.qc_temp_cool_decidegc = 220;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_normal_decidegc",
			&chip->limits.qc_temp_normal_decidegc);
	if (rc < 0) {
		chip->limits.qc_temp_normal_decidegc = 340;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_warm_decidegc",
			&chip->limits.qc_temp_warm_decidegc);
	if (rc < 0) {
		chip->limits.qc_temp_warm_decidegc = 370;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_little_hot_decidegc",
			&chip->limits.qc_temp_little_hot_decidegc);
	if (rc < 0) {
		chip->limits.qc_temp_little_hot_decidegc = 400;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_hot_decidegc",
			&chip->limits.qc_temp_hot_decidegc);
	if (rc < 0)
		chip->limits.qc_temp_hot_decidegc = 420;
	rc = of_property_read_u32(node, "qcom,qc_temp_cool_fastchg_ma",
			&chip->limits.qc_temp_cool_fastchg_ma);
	if (rc) {
		chip->limits.qc_temp_cool_fastchg_ma = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_normal_fastchg_ma_hight",
			&chip->limits.qc_temp_normal_fastchg_ma_hight);
	if (rc) {
		chip->limits.qc_temp_normal_fastchg_ma_hight = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_normal_fastchg_ma_low",
			&chip->limits.qc_temp_normal_fastchg_ma_low);
	if (rc) {
		chip->limits.qc_temp_normal_fastchg_ma_low = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_warm_fastchg_ma",
			&chip->limits.qc_temp_warm_fastchg_ma);
	if (rc) {
		chip->limits.qc_temp_warm_fastchg_ma = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_little_hot_fastchg_ma",
			&chip->limits.qc_temp_little_hot_fastchg_ma);
	if (rc) {
		chip->limits.qc_temp_little_hot_fastchg_ma = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_hot_fastchg_ma",
			&chip->limits.qc_temp_hot_fastchg_ma);
	if (rc < 0) {
		chip->limits.qc_temp_hot_fastchg_ma = 2000;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_hot2_fastchg_ma",
			&chip->limits.qc_temp_hot2_fastchg_ma);
	if (rc < 0) {
		chip->limits.qc_temp_hot2_fastchg_ma = 2000;
	}

	chg_err(
			"qc_temp_cool_decidegc = %d, \
			qc_temp_normal_decidegc = %d, \
			qc_temp_warm_decidegc = %d, \
			qc_temp_cool_fastchg_ma = %d, \
			qc_temp_normal_fastchg_ma_hight = %d, \
			qc_temp_normal_fastchg_ma_low = %d, \
			qc_temp_warm_fastchg_ma = %d, \
			qc_temp_little_hot_fastchg_ma = %d, \
			qc_temp_hot_fastchg_ma = % d, \
			qc_temp_hot2_fastchg_ma = % d\n",
			chip->limits.qc_temp_cool_decidegc,
			chip->limits.qc_temp_normal_decidegc,
			chip->limits.qc_temp_warm_decidegc,
			chip->limits.qc_temp_cool_fastchg_ma,
	...
	chip->unsupport_short_c = of_property_read_bool(node, "qcom,unsupport_short_c");
	chip->qc_9v2a_support = of_property_read_bool(node, "qcom,qc_9v2a_support");
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			...
			chip->ffc_support = %d, \
			chip->unsupport_short_c = %d, \
			chip->qc_9v2a_support = %d\n",
			chip->limits.input_current_charger_ma,
			chip->limits.input_current_usb_ma,
			chip->limits.input_current_led_ma,
			chip->limits.input_current_led_ma_normal,
			...
			chip->limits.charger_current_warp_ma_normal,
			chip->ffc_support,
			chip->unsupport_short_c,
			chip->qc_9v2a_support);
	return 0;
}

static void oneplus_chg_set_charging_current(struct oneplus_chg_chip *chip)
{
	int charging_current = ONEPLUS_CHG_DEFAULT_CHARGING_CURRENT;
	chg_debug("battstatus=%d\n",chip->tbatt_status);
	switch (chip->tbatt_status) {
		...
		case BATTERY_STATUS__LITTLE_COOL_TEMP:
			...
			if (vbatt_higherthan_4180mv)
				...
				chg_debug("littlecool charging_current=%d\n", charging_current);
			break;
		case BATTERY_STATUS__NORMAL:
			if (chip->qc_9v2a_support) {
				chg_debug("9v2a_battstatus=%d\n", chip->tbatt_status_9v2a);
				switch (chip->tbatt_status_9v2a) {
				case BATT_STAT_NORMAL_0:
					chip->limits.temp_normal_fastchg_current_ma =
						chip->limits.qc_temp_cool_fastchg_ma;
					break;
				case BATT_STAT_NORMAL:
					if (vbatt_higherthan_4180mv)
						chip->limits.temp_normal_fastchg_current_ma =
							chip->limits.qc_temp_normal_fastchg_ma_low;
					else
						chip->limits.temp_normal_fastchg_current_ma =
							chip->limits.qc_temp_normal_fastchg_ma_hight;
					break;
				case BATT_STAT_NORMAL_1:
					chip->limits.temp_normal_fastchg_current_ma =
						chip->limits.qc_temp_warm_fastchg_ma;
					break;
				case BATT_STAT_NORMAL_2:
					chip->limits.temp_normal_fastchg_current_ma =
						chip->limits.qc_temp_little_hot_fastchg_ma;
					break;
				case BATT_STAT_NORMAL_3:
					chip->limits.temp_normal_fastchg_current_ma =
						chip->limits.qc_temp_hot_fastchg_ma;
					break;
				case BATT_STAT_NORMAL_4:
					chip->limits.temp_normal_fastchg_current_ma =
						chip->limits.qc_temp_hot2_fastchg_ma;
					break;
				default:
	...
	if (oneplus_short_c_batt_is_prohibit_chg(chip)) {
	...
	if ((chip->led_on) && (charging_current > chip->limits.temp_warm_fastchg_current_ma_led_on)) {
		if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP || chip->tbatt_status_9v2a == BATT_STAT_NORMAL_4) {
...
{
	int current_limit = 0;
	int vbus_limit = VBUS_LIMIT_9V;
	
	if(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC)
	{
		vbus_limit = chip->limits.qc_input_volt_charger_v;
	}
	...
	vbus_limit = g_battery_temp_config[chip->battery_temp_status].vbus;

	if (chip->led_on && !chip->cool_down) {
		if (chip->board_temp_status != BOARD_TEMP_STATUS_INVALID) {
			vbus_limit = g_btemp_config[chip->board_temp_status].vbus;
			if (current_limit > g_btemp_config[chip->board_temp_status].input_current)
				current_limit = g_btemp_config[chip->board_temp_status].input_current;
			charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]BOARD TEMP STATUS CHANGED, board_temp_status=%d,IS ON current=%d\n",
				chip->board_temp_status, current_limit);
		}
		if ((chip->chg_ctrl_by_camera) && (chip->camera_on)
				&& (current_limit > chip->limits.input_current_camera_ma)) {
			current_limit = chip->limits.input_current_camera_ma;
			vbus_limit = VBUS_LIMIT_5V;
			charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]CAMERA STATUS CHANGED, IS ON\n");
		}
	} else if ((chip->chg_ctrl_by_camera) && (chip->camera_on)
			&&(current_limit > chip->limits.input_current_camera_ma)  && !chip->cool_down) {
		current_limit = chip->limits.input_current_camera_ma;
		vbus_limit = VBUS_LIMIT_5V;
		charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]CAMERA STATUS CHANGED, IS ON\n");
	}
	if ((chip->chg_ctrl_by_calling) && (chip->calling_on)
			&& (current_limit > chip->limits.input_current_calling_ma) && !chip->cool_down) {
	...
	charger_xlog_printk(CHG_LOG_CRTI,
		...
		chip->calling_on,
		current_limit,
		chip->board_temp_status,
		vbus_limit);
	chip->chg_ops->input_current_write(current_limit);
	if(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC){
		chg_debug("write vbus = %d", vbus_limit);
		chip->chg_ops->vbus_voltage_write(vbus_limit);
	}
}

static int oneplus_chg_get_float_voltage(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_set_float_voltage(struct oneplus_chg_chip *chip)
{
	int flv = oneplus_chg_get_float_voltage(chip);
...
static void oneplus_chg_vfloat_over_check(struct oneplus_chg_chip *chip)
{
	if (!chip->mmi_chg) {
		return;
	}
	...
	if (oneplus_warp_get_allow_reading() == false) {
		return;
	}
	...
	if (oneplus_warp_get_fastchg_ing() == true) {
		return;
	}
	//if (!((oneplus_warp_get_fastchg_to_normal()== true) || (oneplus_warp_get_fastchg_to_warm() == true))) {
...
static void oneplus_chg_check_vbatt_higher_than_4180mv(struct oneplus_chg_chip *chip)
{
	...
	if (vol_high_pre != vbatt_higherthan_4180mv) {
		...
		oneplus_chg_set_charging_current(chip);
	}
}

#define TEMP_FFC_COUNTS		2
static void oneplus_chg_check_ffc_temp_status(struct oneplus_chg_chip *chip)
{
	int batt_temp = chip->temperature;
	ONEPLUS_CHG_FFC_TEMP_STATUS temp_status = chip->ffc_temp_status;
...
void oneplus_chg_turn_on_ffc1(struct oneplus_chg_chip *chip)
{
	if (!chip->authenticate) {
		return;
	}
	...
	pr_err("oneplus_chg_turn_on_ffc1--------\r\n");
	...
	chip->limits.little_cool_vfloat_over_sw_limit
		= chip->limits.ffc1_normal_vfloat_over_sw_limit;
	oneplus_chg_check_tbatt_status(chip);
	oneplus_chg_set_float_voltage(chip);
	oneplus_chg_set_charging_current(chip);
	oneplus_chg_set_input_current_limit(chip);
	chip->chg_ops->term_current_set(chip->limits.iterm_ma);
}

void oneplus_chg_turn_on_ffc2(struct oneplus_chg_chip *chip)
{
	if (!chip->authenticate) {
		return;
	}
	...
	pr_err("oneplus_chg_turn_on_ffc2--------\r\n");
	chip->chg_ctrl_by_lcd = false;
	chip->fastchg_to_ffc = true;
...
void oneplus_chg_turn_off_charging(struct oneplus_chg_chip *chip)
{
	...
	/*charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] oneplus_chg_turn_off_charging !!\n");*/
}
/*
static int oneplus_chg_check_suspend_or_disable(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_voter_charging_start(struct oneplus_chg_chip *chip,
					ONEPLUS_CHG_STOP_VOTER voter)
{
	chip->chging_on = true;
	...
	switch (voter) {
		case CHG_STOP_VOTER__FULL:
			...
			if (oneplus_warp_get_allow_reading() == true) {
...
static void oneplus_chg_voter_charging_stop(struct oneplus_chg_chip *chip,
					ONEPLUS_CHG_STOP_VOTER voter)
{
	...
	switch (voter) {
		case CHG_STOP_VOTER__FULL:
			...
			if (oneplus_warp_get_allow_reading() == true) {
				if (oneplus_chg_check_suspend_or_disable(chip) == CHG_SUSPEND) {
					chip->chg_ops->charger_suspend();
				} else {
					oneplus_chg_turn_off_charging(chip);
				}
...
static void battery_temp_anti_shake_handle(struct oneplus_chg_chip *chip)
{
...
static OP_CHG_9V2A_TBATT_STATUS batt_temp_9v2a_stat_check(int temp, struct oneplus_chg_chip *chip)
{
	OP_CHG_9V2A_TBATT_STATUS i;

	if (temp >= chip->limits.warm_bat_decidegc && temp <= chip->limits.hot_bat_decidegc) {
		chip->led_temp_change = true;
		return BATT_STAT_WARM;
	}
	chip->led_temp_change = false;
	for (i = BATT_STAT_COLD; i < BATT_STAT_WARM; ++i) {
		if (temp >= *(batt_temp_zone_arr[i].batt_temp_thres_addr) &&
			temp < *(batt_temp_zone_arr[i + 1].batt_temp_thres_addr))
			return i;
	}
	return BATT_STAT_INVALID;
}

static void battery_temp_9v2a_anti_shake_handle(struct oneplus_chg_chip *chip)
{
	int i;

	if (chip->tbatt_status_9v2a == BATT_STAT_NORMAL || chip->tbatt_status_9v2a == BATT_STAT_INVALID) {
		for (i = BATT_STAT_COLD; i < BATT_STAT_INVALID; ++i)
			*(batt_temp_zone_arr[i].batt_temp_thres_addr) = batt_temp_zone_arr[i].default_value;
	} else if (chip->tbatt_status_9v2a < BATT_STAT_NORMAL) {
		for (i = BATT_STAT_COLD; i < BATT_STAT_INVALID; ++i) {
			if (i - 1 == chip->tbatt_status_9v2a)
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) =
					batt_temp_zone_arr[i].default_value + HYSTERISIS_DECIDEGC;
			else
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) = batt_temp_zone_arr[i].default_value;
		}
	} else {
		for (i = BATT_STAT_COLD; i < BATT_STAT_INVALID; ++i) {
			if (i == chip->tbatt_status_9v2a)
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) =
					batt_temp_zone_arr[i].default_value - HYSTERISIS_DECIDEGC;
			else
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) = batt_temp_zone_arr[i].default_value;
...
#define TEMP_CNT	1
static bool oneplus_chg_check_tbatt_is_good(struct oneplus_chg_chip *chip)
{
	static bool ret = true;
	static int temp_counts = 0;
	int batt_temp = chip->temperature;
	ONEPLUS_CHG_TBATT_STATUS tbatt_status = chip->tbatt_status;
	OP_CHG_9V2A_TBATT_STATUS tbatt_status_9v2a;

	if (batt_temp > chip->limits.hot_bat_decidegc || batt_temp < chip->limits.little_cold_bat_decidegc) {
		temp_counts++;
	...
	if (tbatt_status != chip->tbatt_status) {
		if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP
			...
			if (chip->soc != 100 && chip->batt_full == true
				...
				oneplus_chg_voter_charging_start(chip, CHG_STOP_VOTER__FULL);
			}
		}
	...
	if (chip->qc_9v2a_support) {
		tbatt_status_9v2a = batt_temp_9v2a_stat_check(batt_temp, chip);
		if (tbatt_status_9v2a != chip->tbatt_status_9v2a) {
			charger_xlog_printk(CHG_LOG_CRTI, "tbatt_status_9v2a changed, now is %d, pre is %d\n",
								tbatt_status_9v2a, chip->tbatt_status_9v2a);
			chip->tbatt_status_9v2a = tbatt_status_9v2a;
			if (oneplus_warp_get_allow_reading() == true)
				oneplus_chg_set_charging_current(chip);
			battery_temp_9v2a_anti_shake_handle(chip);
		}
	}
	return ret;
}
...
#define VCHG_CNT	2
#define NON_QC_VCHG_OVP_VOL 5800
static bool oneplus_chg_check_vchg_is_good(struct oneplus_chg_chip *chip)
{
	static bool ret = true;
	static int vchg_counts = 0;
	int vol_limit, vol_res;
	int chg_volt = chip->charger_volt;
	ONEPLUS_CHG_VCHG_STATUS vchg_status = chip->vchg_status;

	if (oneplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) {
		return true;
	}
	if (oneplus_warp_get_fastchg_started() == true) {
		return true;
	}
	if (chip->usb_source_changed)
		vchg_counts = 0;
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP || chip->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3) {
		vol_limit = chip->limits.charger_hv_thr;
		vol_res = chip->limits.charger_recv_thr;
	} else {
		vol_limit = NON_QC_VCHG_OVP_VOL;
		vol_res = NON_QC_VCHG_OVP_VOL - 300;
	}
	if (chg_volt > vol_limit) {
		vchg_counts++;
...
static bool oneplus_chg_check_vbatt_is_good(struct oneplus_chg_chip *chip)
{
	static bool ret = true;
...
static bool oneplus_chg_check_time_is_good(struct oneplus_chg_chip *chip)
{
#ifdef SELL_MODE
	/* Qiao.Hu@BSP.BaseDrv.CHG.Basic, 2017/10/26, delete over_time for sell_mode */
	chip->chging_over_time = false;
	printk("oneplus_chg_check_time_is_good_sell_mode\n");
	return true;
#endif //SELL_MODE

#ifdef CONFIG_ONEPLUS_SPECIAL_DEL
	/* Qiao.Hu@BSP.BaseDrv.CHG.Basic, 2019/3/8, delete over_time for aging_mode */
	chip->chging_over_time = false;
	printk("oneplus_chg_check_time_is_good_aging_mode\n");
	return true;
#endif //CONFIG_ONEPLUS_SPECIAL_DEL
...
void oneplus_chg_set_led_status(bool val)
{
	/*Do nothing*/
}
EXPORT_SYMBOL(oneplus_chg_set_led_status);
#else
void oneplus_chg_set_led_status(bool val)
{
...
void oneplus_chg_set_camera_status(bool val)
{
	if (!g_charger_chip) {
		return;
	} else {
...
EXPORT_SYMBOL(oneplus_chg_set_camera_status);

static void op_chg_check_board_temp_status(struct oneplus_chg_chip *chip)
{
	enum BOARD_TEMP_STAT btemp_stat;
	int i;

	if (!chip->led_on)
		btemp_stat = BOARD_TEMP_STATUS_NORMAL;
	else {
		for (i = BOARD_TEMP_STATUS_NORMAL; i < BOARD_TEMP_STATUS_INVALID; ++i) {
			if (chip->board_temp >= g_btemp_config[i].low_thres
				&& chip->board_temp < g_btemp_config[i].high_thres) {
				btemp_stat = i;
				break;
			}
		}
	}
	charger_xlog_printk(CHG_LOG_CRTI, "led_on = %d, board_temp = %d, board_temp_stat = %d, pre_stat = %d\n",
		chip->led_on, chip->board_temp, btemp_stat, chip->board_temp_status);
	if (chip->board_temp_status != btemp_stat) {
		chip->board_temp_status = btemp_stat;
		oneplus_chg_set_input_current_limit(chip);
	}
}
static void op_chg_check_battery_temp_status(struct oneplus_chg_chip *chip)
{
	enum BATTERY_TEMP_STAT battery_temp_stat = BATTERY_TEMP_STATUS_NORMAL;
	int i;

	for (i = BATTERY_TEMP_STATUS_NORMAL; i < BATTERY_TEMP_STATUS_INVALID; ++i) {
		if (chip->temperature >= g_battery_temp_config[i].low_thres
			&& chip->temperature < g_battery_temp_config[i].high_thres) {
			battery_temp_stat = i;
			break;
		}
	}
	charger_xlog_printk(CHG_LOG_CRTI, "temperature = %d, battery_temp_stat = %d, pre_stat = %d\n",
		chip->temperature, battery_temp_stat, chip->battery_temp_status);
	if (chip->battery_temp_status != battery_temp_stat) {
		chip->battery_temp_status = battery_temp_stat;
		oneplus_chg_set_input_current_limit(chip);
	}
}
static void oneplus_chg_check_led_on_ichging(struct oneplus_chg_chip *chip)
{
	if (chip->led_on_change || (chip->led_on && chip->led_temp_change)) {
		...
		if (chip->chg_ctrl_by_warp && chip->vbatt_num == 2) {
			if (oneplus_warp_get_fastchg_started() == true
					&& oneplus_warp_get_fast_chg_type()
...
static void oneplus_chg_check_warp_temp_status(struct oneplus_chg_chip *chip)
{
	int batt_temp = chip->temperature;
	ONEPLUS_CHG_TBAT_WARP_STATUS tbat_warp_status = chip->warp_temp_status;
	static int high_counts = 0, warm_counts = 0, normal_counts = 0;
	static bool warp_first_set_input_current_flag = false;

	if (chip->vbatt_num != 2
			|| oneplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP
			|| oneplus_warp_get_fastchg_started() == false) {
...
static void oneplus_chg_check_warp_ichging(struct oneplus_chg_chip *chip)
{
	if (chip->vbatt_num == 2 && chip->warp_temp_change) {
		chip->warp_temp_change = false;
		if (oneplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP) {
			oneplus_chg_set_input_current_limit(chip);
		}
	}
}

static void oneplus_chg_check_cool_down_ichging(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_check_camera_on_ichging(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_check_calling_on_ichging(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_battery_authenticate_check(struct oneplus_chg_chip *chip)
{
	...
	if (charger_exist_pre ^ chip->charger_exist) {
		...
		if (chip->charger_exist && !chip->authenticate) {
			chip->authenticate = oneplus_gauge_get_batt_authenticate();
		}
	}
}
...
void oneplus_chg_variables_reset(struct oneplus_chg_chip *chip, bool in)
{
	...
	chip->in_rechging = 0;
	chip->limits.qc_input_volt_charger_v = VBUS_LIMIT_9V;
	/*chip->batt_volt = 0;*/
	/*chip->temperature = 0;*/
...
#endif
	oneplus_chg_battery_authenticate_check(chip);
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
	...
	if (chip->qc_9v2a_support) {
		chip->tbatt_status_9v2a = BATT_STAT_NORMAL;
		battery_temp_9v2a_anti_shake_handle(chip);
	}
	chip->board_temp_status = BOARD_TEMP_STATUS_INVALID;
}
...
/*	chip->pmic_spmi.usb_online = false;
	...
	if(chip->external_gauge) {
		chg_debug("use oneplus_gauge_get_batt_authenticate\n");
		chip->authenticate = oneplus_gauge_get_batt_authenticate();
	} else {
		chg_debug("use get_oneplus_high_battery_status\n");
		//chip->authenticate = get_oneplus_high_battery_status();
		chip->authenticate = oneplus_gauge_get_batt_authenticate();
	}
	if (!chip->authenticate) {
		//chip->chg_ops->charger_suspend();
...
//	chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->ffc_temp_status = FFC_TEMP_STATUS__NORMAL;
	chip->warp_temp_status = WARP_TEMP_STATUS__NORMAL;
	chip->short_c_batt.err_code = oneplus_short_c_batt_err_code_init();
	chip->short_c_batt.is_switch_on = oneplus_short_c_batt_chg_switch_init();
	chip->short_c_batt.is_feature_sw_on
		= oneplus_short_c_batt_feature_sw_status_init();
	chip->short_c_batt.is_feature_hw_on
		= oneplus_short_c_batt_feature_hw_status_init();
	chip->short_c_batt.shortc_gpio_status = 1;
	chip->short_c_batt.disable_rechg = false;
	chip->short_c_batt.limit_chg = false;
	chip->short_c_batt.limit_rechg = false;
	chip->usb_source_changed = false;
	if (chip->qc_9v2a_support) {
		batt_temp_zone_arr_init(chip);
		chip->tbatt_status_9v2a = BATT_STAT_NORMAL;
	}
	chip->board_temp_status = BOARD_TEMP_STATUS_INVALID;
}

static void oneplus_chg_fail_action(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_check_rechg_status(struct oneplus_chg_chip *chip)
{
	int recharging_vol;
	...
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) {				//4.0
		recharging_vol = oneplus_chg_get_float_voltage(chip) - 300;
	} else if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {//4.4
		recharging_vol = oneplus_chg_get_float_voltage(chip) - 200;
	} else {
		recharging_vol = oneplus_chg_get_float_voltage(chip);//warm 4.1
	...
	if (oneplus_short_c_batt_is_disable_rechg(chip)) {
	...
	if (rechging_cnt > D_RECHGING_CNT) {
		...
		oneplus_chg_voter_charging_start(chip, CHG_STOP_VOTER__FULL);/*now rechging!*/
	}
}

static void oneplus_chg_full_action(struct oneplus_chg_chip *chip)
{
	charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] Battery full !!\n");
	oneplus_chg_voter_charging_stop(chip, CHG_STOP_VOTER__FULL);
	/*chip->charging_state = CHARGING_STATUS_FULL;*/
	chip->batt_full = true;
	chip->total_time = 0;
	chip->in_rechging = false;
	chip->limits.vfloat_over_counts = 0;
	oneplus_chg_check_rechg_status(chip);
}
...
void oneplus_charger_detect_check(struct oneplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
		oneplus_chg_set_awake(chip, true);
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			oneplus_chg_variables_reset(chip, true);
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
			...
			} else if (oneplus_warp_get_fastchg_started() == false
		...
		oneplus_chg_variables_reset(chip, false);
		if (!chip->mmi_fastchg) {
			oneplus_mmi_fastchg_in(chip);
		}
		oneplus_gauge_set_batt_full(false);
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
		...
		oneplus_chg_set_awake(chip, false);
	}
}
...
static void oneplus_chg_get_battery_data(struct oneplus_chg_chip *chip)
{
	...
	if (oneplus_warp_get_fastchg_started() == true) {
		chip->batt_volt = oneplus_gauge_get_prev_batt_mvolts();
		chip->batt_volt_max = oneplus_gauge_get_prev_batt_mvolts_2cell_max();
		chip->batt_volt_min = oneplus_gauge_get_prev_batt_mvolts_2cell_min();
		chip->icharging = oneplus_gauge_get_prev_batt_current();
		chip->temperature = oneplus_gauge_get_prev_batt_temperature();
		chip->soc = oneplus_gauge_get_prev_batt_soc();
		chip->batt_rm = oneplus_gauge_get_prev_remaining_capacity() * chip->vbatt_num;
	} else {
		chip->batt_volt = oneplus_gauge_get_batt_mvolts();
		chip->batt_volt_max = oneplus_gauge_get_batt_mvolts_2cell_max();
		chip->batt_volt_min = oneplus_gauge_get_batt_mvolts_2cell_min();
		chip->icharging = oneplus_gauge_get_batt_current();
		chip->temperature = oneplus_gauge_get_batt_temperature();
		chip->soc = oneplus_gauge_get_batt_soc();
		chip->batt_fcc = oneplus_gauge_get_batt_fcc() * chip->vbatt_num;
		chip->batt_cc = oneplus_gauge_get_batt_cc() * chip->vbatt_num;
		chip->batt_soh = oneplus_gauge_get_batt_soh();
		chip->batt_rm = oneplus_gauge_get_remaining_capacity() * chip->vbatt_num;
		chip->board_temp = get_charging_therm_temp()/1000;
		if(chip->external_gauge)
		{
			oneplus_gauge_get_batt_fc();
			oneplus_gauge_get_batt_qm();
			oneplus_gauge_get_batt_pd();
			oneplus_gauge_get_batt_rcu();
			oneplus_gauge_get_batt_rcf();
			oneplus_gauge_get_batt_fcu();
			oneplus_gauge_get_batt_fcf();
			oneplus_gauge_get_batt_sou();
			oneplus_gauge_get_batt_do0();
			oneplus_gauge_get_batt_doe();
			oneplus_gauge_get_batt_trm();
			oneplus_gauge_get_batt_pc();
			oneplus_gauge_get_batt_qs();
		}
	}
	...
	if (ui_soc_cp_flag == 0) {
		if ((chip->soc < 0 || chip->soc > 100) && retry_counts < RETRY_COUNTS) {
			charger_xlog_printk(CHG_LOG_CRTI,
				"[Battery]oneplus_chg_get_battery_data,\
		...
		if ((soc_load != 0) && ((abs(soc_load-chip->soc)) <= 10)) {
...
/*need to extend it*/
static void oneplus_chg_set_aicl_point(struct oneplus_chg_chip *chip)
{
	if (oneplus_warp_get_allow_reading() == true) {
...
static void oneplus_chg_check_aicl_input_limit(struct oneplus_chg_chip *chip)
{
	...
	if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true
			|| ((chip->tbatt_status != BATTERY_STATUS__NORMAL)
			&& (chip->tbatt_status != BATTERY_STATUS__LITTLE_COOL_TEMP))
			|| chip->ui_soc > 85 || oneplus_warp_get_fastchg_started() == true) {
		aicl_delay_count = 0;
		return;
	}
...
static void oneplus_chg_aicl_check(struct oneplus_chg_chip *chip)
{
	if (oneplus_warp_get_fastchg_started() == false) {
		oneplus_chg_set_aicl_point(chip);
		oneplus_chg_check_aicl_input_limit(chip);
	}
}

static void oneplus_chg_protection_check(struct oneplus_chg_chip *chip)
{
	if (false == oneplus_chg_check_tbatt_is_good(chip)) {
		chg_err("oneplus_chg_check_tbatt_is_good func ,false!\n");
		oneplus_chg_voter_charging_stop(chip, CHG_STOP_VOTER__BATTTEMP_ABNORMAL);
	} else {
		if ((chip->stop_voter & CHG_STOP_VOTER__BATTTEMP_ABNORMAL)
				== CHG_STOP_VOTER__BATTTEMP_ABNORMAL) {
			charger_xlog_printk(CHG_LOG_CRTI,
				"oneplus_chg_check_tbatt_is_good func ,true! To Normal\n");
			oneplus_chg_voter_charging_start(chip, CHG_STOP_VOTER__BATTTEMP_ABNORMAL);
		}
	}
	if (false == oneplus_chg_check_vchg_is_good(chip)) {
		chg_err("oneplus_chg_check_vchg_is_good func ,false!\n");
		oneplus_chg_voter_charging_stop(chip, CHG_STOP_VOTER__VCHG_ABNORMAL);
	} else {
		if ((chip->stop_voter & CHG_STOP_VOTER__VCHG_ABNORMAL)
				== CHG_STOP_VOTER__VCHG_ABNORMAL) {
			charger_xlog_printk(CHG_LOG_CRTI,
				"oneplus_chg_check_vchg_is_good func ,true! To Normal\n");
			oneplus_chg_voter_charging_start(chip, CHG_STOP_VOTER__VCHG_ABNORMAL);
		}
	}
#ifdef FEATURE_VBAT_PROTECT
	if (false == oneplus_chg_check_vbatt_is_good(chip)) {
		chg_err("oneplus_chg_check_vbatt_is_good func ,false!\n");
		oneplus_chg_voter_charging_stop(chip, CHG_STOP_VOTER__VBAT_TOO_HIGH);
	}
#endif
	if (false == oneplus_chg_check_time_is_good(chip)) {
		chg_err("oneplus_chg_check_time_is_good func ,false!\n");
		oneplus_chg_voter_charging_stop(chip, CHG_STOP_VOTER__MAX_CHGING_TIME);
	}
	oneplus_chg_check_vbatt_higher_than_4180mv(chip);
	oneplus_chg_vfloat_over_check(chip);
	oneplus_chg_check_ffc_temp_status(chip);
	op_chg_check_board_temp_status(chip);

	op_chg_check_battery_temp_status(chip);
	oneplus_chg_check_led_on_ichging(chip);
	if (chip->chg_ctrl_by_camera) {
		oneplus_chg_check_camera_on_ichging(chip);
	}
	if (chip->chg_ctrl_by_calling) {
		oneplus_chg_check_calling_on_ichging(chip);
	}
	if (chip->chg_ctrl_by_warp) {
		oneplus_chg_check_warp_temp_status(chip);
		oneplus_chg_check_warp_ichging(chip);
	}
	if (chip->cool_down_done) {
		oneplus_chg_check_cool_down_ichging(chip);
	}
}


static void battery_notify_tbat_check(struct oneplus_chg_chip *chip)
{
...
static void battery_notify_authenticate_check(struct oneplus_chg_chip *chip)
{
	if (!chip->authenticate) {
		chip->notify_code |= 1 << NOTIFY_BAT_NOT_CONNECT;
...
static void battery_notify_vcharger_check(struct oneplus_chg_chip *chip)
{
...
static void battery_notify_vbat_check(struct oneplus_chg_chip *chip)
{
	static int count = 0;
...
static void battery_notify_max_charging_time_check(struct oneplus_chg_chip *chip)
{
...
static void battery_notify_short_c_battery_check(struct oneplus_chg_chip *chip)
{
...
static void battery_notify_flag_check(struct oneplus_chg_chip *chip)
{
...
static void battery_notify_charge_terminal_check(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_battery_notify_check(struct oneplus_chg_chip *chip)
{
...
static bool oneplus_chg_soc_reduce_slow_when_1(struct oneplus_chg_chip *chip)
{
...
static void oneplus_chg_update_ui_soc(struct oneplus_chg_chip *chip)
{
	...
	int pre_prop_status = chip->prop_status;
	...
	if (chip->ui_soc < 2) {
		if (oneplus_chg_soc_reduce_slow_when_1(chip) == true) {
			chip->ui_soc = 0;
		} else {
			chip->ui_soc = 1;
		}
	...
	if (chip->prop_status != pre_prop_status)
		power_supply_changed(chip->batt_psy);
}

static void fg_update(struct oneplus_chg_chip *chip)
{
...
static void battery_update(struct oneplus_chg_chip *chip)
{
	static int iWakeUpDelay = 0;
	static struct power_supply *bms_psy;
	int ui_soc_local = chip->ui_soc;
	int prop_status_local = chip->prop_status;

	if (!bms_psy) {
		bms_psy = power_supply_get_by_name("bms");
		charger_xlog_printk(CHG_LOG_CRTI, "bms_psy null\n");
	}
	oneplus_chg_update_ui_soc(chip);
	if ((ui_soc_local != chip->ui_soc || prop_status_local != chip->prop_status) && bms_psy)
		power_supply_changed(bms_psy);
	if (chip->fg_bcl_poll) {
		fg_update(chip);
	}
	...
	{
		iWakeUpDelay++;
		if(iWakeUpDelay >= ONEPLUS_CHGON_WAKEUP_DELAY_COUNT)
		{
			iWakeUpDelay = 0;
		}
		else
...
static void oneplus_chg_get_chargerid_voltage(struct oneplus_chg_chip *chip)
{
	...
	if (oneplus_warp_get_warp_switch_val() == 1) {
		if (chip->chargerid_volt_got == false) {
			chip->chg_ops->set_chargerid_switch_val(1);
			oneplus_warp_set_warp_chargerid_switch_val(1);
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
			if (oneplus_warp_get_fastchg_started() == false){
				oneplus_warp_switch_mode(NORMAL_CHARGER_MODE);
			}
			...
			oneplus_warp_set_warp_chargerid_switch_val(0);
		} else {
	...
	} else if (oneplus_warp_get_warp_switch_val() == 0) {
		if (chip->chargerid_volt_got == false) {
			chip->chg_ops->set_chargerid_switch_val(1);
			oneplus_warp_set_warp_chargerid_switch_val(1);
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
...
static void oneplus_chg_chargerid_switch_check(struct oneplus_chg_chip *chip)
{
	return oneplus_chg_get_chargerid_voltage(chip);
}

#define RESET_MCU_DELAY_15S		3

static void oneplus_chg_fast_switch_check(struct oneplus_chg_chip *chip)
{
	if (oneplus_short_c_batt_is_prohibit_chg(chip)) {
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oneplus_warp_get_fastchg_started() == false
			...
			oneplus_warp_switch_fast_chg();
		}
		if (!oneplus_warp_get_fastchg_started()
				&& !oneplus_warp_get_fastchg_dummy_started()
				&& !oneplus_warp_get_fastchg_to_normal()
				&& !oneplus_warp_get_fastchg_to_warm()) {
			reset_mcu_delay++;
			if (reset_mcu_delay == RESET_MCU_DELAY_15S) {
				charger_xlog_printk(CHG_LOG_CRTI, "  reset mcu again\n");
				oneplus_warp_set_ap_clk_high();
				oneplus_warp_reset_mcu();
				...
				if (chip->charger_volt <= 7500) {
					oneplus_warp_reset_fastchg_after_usbout();
					chip->chg_ops->set_chargerid_switch_val(0);
...
static int oneplus_chg_check_sw_full(struct oneplus_chg_chip *chip)
{
...
static int oneplus_chg_check_hw_full(struct oneplus_chg_chip *chip)
{
	...
	vbatt_full_vol_hw = oneplus_chg_get_float_voltage(chip);
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) {
...
static void oneplus_chg_ffc_variable_reset(struct oneplus_chg_chip *chip)
{
	chip->fastchg_to_ffc = false;
	chip->fastchg_ffc_status = 0;
	chip->chg_ctrl_by_lcd = chip->chg_ctrl_by_lcd_default;
...
static bool oneplus_chg_check_ffc_status(struct oneplus_chg_chip *chip)
{
			...
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging * -1) < (chip->limits.ff1_normal_fastchg_ma
					...
					if (normal_counts >= FFC_CURRENT_COUNTS) {
						oneplus_chg_ffc_variable_reset(chip);
						oneplus_chg_turn_on_charging(chip);
						return true;
					}
				} else {
...
static bool oneplus_chg_check_vbatt_is_full_by_sw(struct oneplus_chg_chip *chip)
{
	bool ret_sw = false;
	bool ret_hw = false;

	if (chip->vbatt_over || !chip->check_batt_full_by_sw) {
		return false;
	}
	ret_sw = oneplus_chg_check_sw_full(chip);
	ret_hw = oneplus_chg_check_hw_full(chip);
...
static void oneplus_chg_check_status_full(struct oneplus_chg_chip *chip)
{
	int is_batt_full = 0;
	static int fastchg_present_wait_count = 0;

	if (chip->chg_ctrl_by_warp) {
		if (oneplus_warp_get_fastchg_ing()
				== true && oneplus_warp_get_fast_chg_type()
				!= CHARGER_SUBTYPE_FASTCHG_WARP)
			return;
	} else {
		if (oneplus_warp_get_fastchg_ing() == true)
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		if (chip->recharge_after_full == true || chip->vbatt_num == 2) {
			...
			if (chip->dod0_counts == DOD0_COUNTS) {
				if (chip->vbatt_num == 2) {
					oneplus_gauge_update_battery_dod0();
					//chip->in_rechging = true;
					//oneplus_chg_voter_charging_start(chip, CHG_STOP_VOTER__FULL);/*now rechging!*/
					charger_xlog_printk(CHG_LOG_CRTI, "oneplus_chg_check_status_full,dod0_counts = %d\n", chip->dod0_counts);
				}
		...
		oneplus_chg_fail_action(chip);
	} else {
		chip->charging_state = CHARGING_STATUS_CCCV;
	}
}

static void oneplus_chg_kpoc_power_off_check(struct oneplus_chg_chip *chip)
{
	...
	if (chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		...
		if ((chip->chg_ops->check_chrdet_status() == false)
			...
			if ((oneplus_warp_get_fastchg_to_normal() == false)
					&& (oneplus_warp_get_fastchg_to_warm() == false)
					&& (oneplus_warp_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE)
					&& (oneplus_warp_get_btb_temp_over() == false)) {
				charger_xlog_printk(CHG_LOG_CRTI,
...
static void oneplus_chg_print_log(struct oneplus_chg_chip *chip)
{
#ifdef T0_ERROR_BATT_SUPPORT
	if (is_t0_error_batt)
		charger_xlog_printk(CHG_LOG_CRTI, "------------------------WARNING:This is N1's t0 error battery!------------------------\n");
#endif
	...
	if(chip->external_gauge){
		charger_xlog_printk(CHG_LOG_CRTI,
			...
			oneplus_gauge_get_batt_fc(),oneplus_gauge_get_batt_qm(),
			oneplus_gauge_get_batt_pd(),oneplus_gauge_get_batt_rcu(),
			oneplus_gauge_get_batt_rcf(),oneplus_gauge_get_batt_fcu(),
			oneplus_gauge_get_batt_fcf(),oneplus_gauge_get_batt_sou(),
			oneplus_gauge_get_batt_do0(),oneplus_gauge_get_batt_doe(),
			oneplus_gauge_get_batt_trm(),oneplus_gauge_get_batt_pc(),
			oneplus_gauge_get_batt_qs(),
...
static void oneplus_chg_critical_log(struct oneplus_chg_chip *chip)
{
	...
	} else if (oneplus_warp_get_btb_temp_over() == true
			|| oneplus_warp_get_fastchg_to_normal() == true) {
...
static void oneplus_chg_other_thing(struct oneplus_chg_chip *chip)
{
	if (oneplus_warp_get_fastchg_started() == false) {
	...
	if (chip->charger_exist) {
		chip->total_time += ONEPLUS_CHG_UPDATE_INTERVAL_SEC;
	}
	oneplus_chg_print_log(chip);
	oneplus_chg_critical_log(chip);
}

#define IBATT_COUNT	10

static void oneplus_chg_ibatt_check_and_set(struct oneplus_chg_chip *chip)
{
	...
	if (!chip->charger_exist || (oneplus_warp_get_fastchg_started() == true)) {
		current_adapt = 0;
		ibatt_count = 0;
		average_current = 0;
		fail_count = 0;
...
static void oneplus_chg_pd_config(struct oneplus_chg_chip *chip)
{
	...
	if (!chip->chg_ops->oneplus_chg_pd_setup
			|| !chip->chg_ops->oneplus_chg_get_pd_type) {
		return;
	}
	if (pd_chging == false && chip->chg_ops->oneplus_chg_get_pd_type() == true) {
		pd_chging = true;
		ret = chip->chg_ops->oneplus_chg_pd_setup();
		if (ret >= 0) {
			chip->limits.temp_little_cool_fastchg_current_ma
				= chip->limits.pd_temp_little_cool_fastchg_current_ma;
			chip->limits.temp_normal_fastchg_current_ma
...
static void oneplus_chg_pdqc_to_normal(struct oneplus_chg_chip *chip)
{
	int ret = 0;
	...
	if (!chip->chg_ops->oneplus_chg_pd_setup || !chip->chg_ops->set_qc_config) {
		return;
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oneplus_chg_chip *chip = container_of(dwork, struct oneplus_chg_chip, update_work);
	static struct power_supply *bms_psy;
	int batt_temp = 0;

	oneplus_charger_detect_check(chip);
	oneplus_chg_get_battery_data(chip);
	if (chip->charger_exist) {
		oneplus_chg_aicl_check(chip);
		oneplus_chg_protection_check(chip);
		oneplus_chg_check_status_full(chip);
		oneplus_chg_battery_notify_check(chip);
		oneplus_chg_get_chargerid_voltage(chip);
		oneplus_chg_fast_switch_check(chip);
		oneplus_chg_chargerid_switch_check(chip);
	}
	oneplus_chg_pd_config(chip);
	oneplus_chg_pdqc_to_normal(chip);
	oneplus_chg_ibatt_check_and_set(chip);
	/* oneplus_chg_short_c_battery_check(chip); */
	if(!chip->unsupport_short_c){
		wake_up_process(chip->shortc_thread);
	}
	oneplus_chg_battery_update_status(chip);
	oneplus_chg_kpoc_power_off_check(chip);
	oneplus_chg_other_thing(chip);
	/* run again after interval */
	batt_temp = oneplus_gauge_get_batt_temperature();
	charger_xlog_printk(CHG_LOG_CRTI, "oneplus_healthd_update temp:%d\n", batt_temp);

	if (batt_temp >= ONEPLUS_BAT_TEMP) {
		power_supply_changed(chip->batt_psy);
		power_supply_changed(chip->usb_psy);
		if (!bms_psy) {
			bms_psy = power_supply_get_by_name("bms");
		...
		if (bms_psy) {
			power_supply_changed(bms_psy);
			charger_xlog_printk(CHG_LOG_CRTI, "oneplus_healthd_update\n");
		}
	}
	schedule_delayed_work(&chip->update_work, ONEPLUS_CHG_UPDATE_INTERVAL);
...
void oneplus_chg_soc_update_when_resume(unsigned long sleep_tm_sec)
{
	...
	g_charger_chip->soc = oneplus_gauge_get_batt_soc();
	oneplus_chg_update_ui_soc(g_charger_chip);
}
...
int oneplus_chg_show_warp_logo_ornot(void)
{
	...
	if (oneplus_warp_get_fastchg_started()) {
		return 1;
	} else if (oneplus_warp_get_fastchg_to_normal() == true
			|| oneplus_warp_get_fastchg_to_warm() == true
			|| oneplus_warp_get_fastchg_dummy_started() == true
			|| oneplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) {
		if (g_charger_chip->prop_status != POWER_SUPPLY_STATUS_FULL
				&&(g_charger_chip->stop_voter == CHG_STOP_VOTER__FULL
				|| g_charger_chip->stop_voter == CHG_STOP_VOTER_NONE)) {
...
void oneplus_smart_charge_by_cool_down(struct oneplus_chg_chip *chip, int val)
{
	...
	if (!chip->chg_ctrl_by_warp)
	{
		...
		charger_xlog_printk(CHG_LOG_CRTI, "set cool_down = [%d].val=0x%x\n", chip->cool_down,val);
		switch ( val )
		{
			case SMART_QC_CHARGER_5V500MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_500_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_500_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_500_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_5V900MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_900_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_900_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_900_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_5V1500MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_5V1200MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_1200_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_1200_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_1200_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_5V1400MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_1400_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_1400_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_1400_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_5V2000MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			case SMART_QC_CHARGER_9V1500MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_1500_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_9V;
				break;
			case SMART_QC_CHARGER_9V2000MA:
				chip->limits.input_current_charger_ma    = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = ONEPLUS_CHG_2000_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_9V;
		...
		if(!(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC))
		{
			if(chip->limits.qc_input_volt_charger_v > VBUS_LIMIT_5V)
			{
				chip->limits.qc_input_volt_charger_v = VBUS_LIMIT_5V;
				chg_debug("ac is not qc but vbus set 9v\n");
			}
		}

		//chip->chg_ops->input_current_write(chip->limits.qc_input_current_charger_ma);
		//chip->chg_ops->vbus_voltage_write(chip->limits.qc_input_volt_charger_v);
		charger_xlog_printk(CHG_LOG_CRTI, "set current limit = [%d]\n", chip->limits.qc_input_current_charger_ma);
		charger_xlog_printk(CHG_LOG_CRTI, "set vbus limit = [%d]\n", chip->limits.qc_input_volt_charger_v);
	}
	else
	{
		...
		if (val & SMART_COMPATIBLE_WARP_CHARGER_CURRENT_BIT0) {
			chip->limits.input_current_warp_ma_high = ONEPLUS_CHG_1800_CHARGING_CURRENT;
			chip->limits.input_current_warp_ma_warm = ONEPLUS_CHG_1800_CHARGING_CURRENT;
			chip->limits.input_current_warp_ma_normal = ONEPLUS_CHG_1800_CHARGING_CURRENT;
			chip->cool_down_done = true;
		}
		if (!val) {
			chip->cool_down = 0;
...
{
return false;//debug for bring up 
#if 0
...
#else
	if (boot_mode == MSM_BOOT_MODE_RF || boot_mode == MSM_BOOT_MODE_WLAN
...
#ifdef CONFIG_ONEPLUS_CHARGER_MTK
/*
int get_oneplus_short_check_fast_to_normal(void)
{
	if (!g_charger_chip) {
		return 0;
	} else {
...
*/
int oneplus_get_prop_status(void)
{
	if (!g_charger_chip) {
		return 0;
	} else {
...
#define ONEPLUS_TBATT_HIGH_PWROFF_COUNT	(18)
#define ONEPLUS_TBATT_EMERGENCY_PWROFF_COUNT	(6)

DECLARE_WAIT_QUEUE_HEAD(oneplus_tbatt_pwroff_wq);

static int oneplus_tbatt_power_off_kthread(void *arg)
{
	int over_temp_count = 0, emergency_count = 0;
	int batt_temp = 0;
	//struct oneplus_chg_chip *chip = (struct oneplus_chg_chip *)arg;
	...
	while (!kthread_should_stop()) {
		...
		if (!tbatt_pwroff_enable) {
			emergency_count = 0;
			over_temp_count = 0;
			wait_event_interruptible(oneplus_tbatt_pwroff_wq, tbatt_pwroff_enable == 1);
		}
		if (oneplus_warp_get_fastchg_started() == true) {
			batt_temp = oneplus_gauge_get_prev_batt_temperature();
		} else {
			batt_temp = oneplus_gauge_get_batt_temperature();
		}
		...
		if (over_temp_count >= ONEPLUS_TBATT_HIGH_PWROFF_COUNT
			|| emergency_count >= ONEPLUS_TBATT_EMERGENCY_PWROFF_COUNT) {
...
int oneplus_tbatt_power_off_task_init(struct oneplus_chg_chip *chip)
{
	...
	chip->tbatt_pwroff_task
		= kthread_create(oneplus_tbatt_power_off_kthread, chip, "tbatt_pwroff");
...
{
	wake_up_interruptible(&oneplus_tbatt_pwroff_wq);
	return;
}
...
void oneplus_chg_set_input_current_without_aicl(int current_ma)
{
    if (!g_charger_chip) {
        ...
        if(g_charger_chip->chg_ops->input_current_write_without_aicl && oneplus_warp_get_allow_reading() == true) {
